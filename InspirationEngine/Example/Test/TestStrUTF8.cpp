...
#include "../../InspirationEngine/Util/StrUTF8.h"
...
void TestStrUTF8::testIncompleteMutliByte()
{
	...
	checkSize("getMemoryPoint(1) 'Hello\\xC0'", StrUTF8::getMemoryPoint(s1, 1), 1u);

	std::string s2 = "Hi\xE0\x80";
	checkSize("getMemoryPoint(1) 'Hi\\xE0\\x80'", StrUTF8::getMemoryPoint(s2, 1), 1u);
...
void TestStrUTF8::testInvalidStartByte()
{
	...
	checkSize("getMemoryPoint(2) 'Test\\xF5...'", StrUTF8::getMemoryPoint(s, 2), 2u);
...
void TestStrUTF8::testValidUTF8()
{
	...
	checkSize("getMemoryPoint(6) → byte 8 (나)", StrUTF8::getMemoryPoint(s, 6), 8u);
...
void TestStrUTF8::testOverlongEncoding()
{
	...
	checkSize("getMemoryPoint(1) 'Test\\xC0\\x80End'", StrUTF8::getMemoryPoint(s, 1), 1u);
...
void TestStrUTF8::testConsecutiveInvalidBytes()
{
	...
	checkSize("getMemoryPoint(2) 'Start\\x80\\x81\\x82End'", StrUTF8::getMemoryPoint(s, 2), 2u);

	std::string t = s;
	StrUTF8::removeToBack(t, 5, 3);
...
void TestStrUTF8::testCJK()
{
	...
	checkSize("getMemoryPoint(2) 你好→ byte 6", StrUTF8::getMemoryPoint(chinese, 2), 6u);
	...
	checkSize("getMemoryPoint(1) 日→ byte 3", StrUTF8::getMemoryPoint(japanese, 1), 3u);

	std::string mixed = "Hi\xE4\xBD\xA0\xE5\xA5\xBD";
	checkSize("getMemoryPoint(3) Hi你→ byte 5", StrUTF8::getMemoryPoint(mixed, 3), 5u);
...
void TestStrUTF8::testArabic()
{
	...
	checkSize("getMemoryPoint(2) مر→ byte 4", StrUTF8::getMemoryPoint(arabic, 2), 4u);
