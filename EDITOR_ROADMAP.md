...
| E-2 파일 브라우저 | ✅ | 경로 내 파일/폴더 트리 표시, Atlas Editor 연동 |
| E-3 뷰포트 | ✅ | IEScene 실시간 렌더링, RMB 패닝, 스크롤 줌, LMB 오브젝트 선택 |
...
**신규 파일**
- `InspirationEditor/IEAtlasEditorWindow.h` — IEWindow 서브클래스, TileDef 구조체
- `InspirationEditor/IEAtlasEditorWindow.cpp` — 전체 구현

**수정 파일**
- `InspirationEditor/main.cpp` — IEAtlasEditorWindow 사용, 폰트 로드, 텍스트 입력 처리
- `InspirationEditor/InspirationEditor.vcxproj` — 새 파일 등록
...
### UI 레이아웃 (1024×720 단일 창)
```
┌────────────────────────────────────────────────────────────┐
│ PNG: [경로______________] W:[32] H:[16]        [Load]      │  ← Header (44px)
├──────────────────────────────────┬─────────────────────────┤
│                                  │ name: [______________]  │
│   PNG 캔버스 (764×632)           │ x:    [______________]  │
│   그리드(회색)                   │ y:    [______________]  │
│   선택 타일(빨간 테두리)         │ w:    [______________]  │
│                                  │ h:    [______________]  │
│                                  │ pivX: [______________]  │
│                                  │ pivY: [______________]  │
│                                  │                         │
│                                  │        [Apply]          │
├──────────────────────────────────┴─────────────────────────┤
│ [Save JSON]   (상태 메시지)            tiles: N            │  ← Footer (44px)
└────────────────────────────────────────────────────────────┘
```

### 기능 목록
- PNG 로드 (`IMG_Load` → SDL_Texture), 캔버스 내 비율 유지 축소/확대
- W/H 입력 → 자동 분할 (`tile_r{r}_c{c}` 이름 부여)
- 타일 클릭 → 선택 강조 (빨간 테두리)
- 속성 패널에서 이름/좌표 편집 + [Apply]
- [Save JSON] → `nlohmann::json` 직렬화, PNG와 동일 폴더에 저장
